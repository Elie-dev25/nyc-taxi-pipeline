-- ============================================================================
-- NYC TAXI PIPELINE - TRANSFORMATION GREEN (BRONZE → SILVER)
-- ============================================================================
USE DATABASE nyc_taxi_db;
USE SCHEMA silver;
USE WAREHOUSE nyc_transform_wh;

TRUNCATE TABLE green_trips_clean;

INSERT INTO green_trips_clean (
    vendor_id,
    pickup_datetime,
    dropoff_datetime,
    passenger_count,
...
WITH converted AS (
    ...
    SELECT
        ...
        TO_TIMESTAMP_NTZ(lpep_pickup_datetime / 1000000)  AS pickup_datetime,
        TO_TIMESTAMP_NTZ(lpep_dropoff_datetime / 1000000) AS dropoff_datetime,
        passenger_count,
        trip_distance,
        ...
        TIMESTAMPDIFF(SECOND,
            TO_TIMESTAMP_NTZ(lpep_pickup_datetime / 1000000),
            TO_TIMESTAMP_NTZ(lpep_dropoff_datetime / 1000000)
        ) / 60.0   AS trip_duration_minutes,
        TIMESTAMPDIFF(SECOND,
            TO_TIMESTAMP_NTZ(lpep_pickup_datetime / 1000000),
            TO_TIMESTAMP_NTZ(lpep_dropoff_datetime / 1000000)
        ) / 3600.0 AS trip_duration_hours,
        HOUR(TO_TIMESTAMP_NTZ(lpep_pickup_datetime / 1000000))    AS pickup_hour,
        DAYNAME(TO_TIMESTAMP_NTZ(lpep_pickup_datetime / 1000000)) AS pickup_day_of_week,
        DATE(TO_TIMESTAMP_NTZ(lpep_pickup_datetime / 1000000))    AS pickup_date,
        MONTH(TO_TIMESTAMP_NTZ(lpep_pickup_datetime / 1000000))   AS pickup_month,
        YEAR(TO_TIMESTAMP_NTZ(lpep_pickup_datetime / 1000000))    AS pickup_year,
        _source_file,
...
-- VÉRIFICATIONS
-- ============================================================================
SELECT COUNT(*) AS total_rows FROM green_trips_clean;
